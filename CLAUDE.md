...
- **Запись в карту гейтится**: лидар пишется только при НОВОМ обороте (`revolution_count`) и |ω| ≤ 0.6 рад/с (точки оборота копятся ~150 мс и при вращении мажутся), УЗ — не чаще 2 Гц; векторная запись `update_cells_batch` (поштучный `update_cell` ел 0.1–0.4 с на тик). Известное ограничение: free-лучи лидара постепенно стирают камерные «низкие» препятствия из карты — защита от них в реактивном слое, не в A*.
