...
def measure_wheel_base() -> float:
    """Колёсная база — без автоматики, только запрос значения линейкой."""
    print("\n" + "=" * 60)
    print("КОЛЁСНАЯ БАЗА")
    print("=" * 60)
    print("Замерьте расстояние между центрами левого и правого колеса (в мм).")
    raw = input("Колёсная база, мм (Enter — пропуск): ").strip()
    if not raw:
        return 0.0
    try:
