...
class NavigationController:
    ...
    # Yahboom не публикует точных паспортных значений; ниже — середина типовых
    # диапазонов для этой платформы. На ровном полу при заряженной батарее ошибка
    # ~±25%, на ковре/подсевшей батарее реальная скорость ниже в 1.5–2 раза.
    # Для серьёзных карт обязательна калибровка скриптом map_scripts/calibrate_odometry.py.
    PWM_FULL = 255.0
